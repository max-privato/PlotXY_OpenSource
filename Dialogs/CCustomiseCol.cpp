...
CCustomiseCol::CCustomiseCol(QWidget *parent) :
  ...
  ui->customColorTable->item(0,numCols-1)->setTextColor(QColor(0,0,0));
  ui->stdColorTable->item(0,numCols-1)->setFlags(Qt::NoItemFlags);
  ui->stdColorTable->item(0,numCols-1)->setTextColor(QColor(0,0,0));
}
...
QList <QColor> CCustomiseCol::setColorf(){
  QList <QColor> colors;
  for(int column=0; column<ui->customColorTable->columnCount()-1; column++){
    colors.append(ui->customColorTable->item(0,column)->backgroundColor());
  }
  return colors;
}
...
void CCustomiseCol::on_stdColorTable_cellPressed(int row, int column)
{
    ...
    setColor=ui->stdColorTable->item(row,column)->backgroundColor();
...
void CCustomiseCol::on_buttonBox_clicked(QAbstractButton *button){
    if(ui->buttonBox->buttonRole(button)==QDialogButtonBox::ResetRole){
      for (int column=0; column<ui->customColorTable->columnCount()-1; column++){
        ui->customColorTable->item(0,column)->
           setBackgroundColor(ui->stdColorTable->item(0,column)->backgroundColor());
...
void CCustomiseCol::on_customColorTable_entered(const QModelIndex) {
    /* può accadere che quando faccio un drop, se lo faccio in fondo alla tabella viene inserita una riga aggiuntiva. Non si vuole assolutamente che ciò accada. Una maniera seria di evitarlo consiste nel modificare l'evento"dropItem della tabella, ma per far questo devo ereditare una versione custom della tabella. Per il momento adotto una tecnica semplificata: manualmente riporto sempre a 1 il numero di colonne.
    */
    ui->stdColorTable->clearSelection();
...
void CCustomiseCol::on_customColorTable_cellDoubleClicked(int row, int column)
{
    ...
    if(ret==QDialog::Accepted){
      ui->customColorTable->item(0,column)-> setBackgroundColor(dialog.currentColor());
      ui->customColorTable->clearSelection();
    }
    /* Devo se faccio un doppio click devo annullare l'effetto del primo dei sue click sollo stato del checked:*/
