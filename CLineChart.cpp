...
int CLineChart::smartDrawUnit(QPainter * myPainter, QFont baseFont, int X, int Y, EadjustType hAdjust, EadjustType vAdjust, QString text,  bool addBrackets, bool Virtual ){
   ...
   * oggetto a cui appartiene, che non sia passata fra i parmetri.
   * In tal modo la riutilizzabilità in LineChart è grandemente facilitata.
   * ***
   *
...
//              setToolTip(lCurveParam[hovData.iTotPlot].fullName);
      ...
      qDebug()<<"sX:"<<sX<<"sY:"<<sY<<" ttType="<<ttType;
 ...
 */
  ...
  for(iFile=0; iFile<nFiles; iFile++){
    int nPoints=filesInfo[iFile].numOfPoints;
    /* Prima di tutto trovo index che è l'indice del valore sull'asse x più vicino l cursore*/
    if(filesInfo[iFile].variableStep){
...
void CLineChart::keyPressEvent(QKeyEvent * event){
  /* In questa funzione implmento lo spostamento del cursore di visualizzazione dei
  ...
  */
  ...
  //Calcolo i valori e lli mando fuori:
  SXYValues values;
...
*/
    ...
    int x=event->pos().x();
    hovVarRect=QRect(0,0,0,0);
    //La seguente sezione 1 + stata trasferita all'interno della funzione event (che cestisce anche lo snat to grid)
    /*
//1) visualizzazioneinformazioni aggiuntive quando si fa l'hovering sul nome di variabile
    setCursor(Qt::ArrowCursor);
   ...
   //A questo punto non sto selezionando il rettangolo di zoom. Adesso se sono in un'azione di dataRectDragging aggiorno i valori numerici e la posizione del cursore dati; altrimenti valuto se il mouse è nel raggio di azione di uno dei tre cursori dati. La cosa viene ripetuta tre volte, una per ogni cursore dati.
    ...
    if(dataCursVisible){
      if(dataCursDragging){
        ...
        //la seguente chiamata attribuisce un valore a nearX:
        values=giveValues(x-1, linearInterpolate, nearX, false, false);
        if(nearX<X0)nearX=X0;
        if(nearX>X1) nearX=X1;
  ...
  //Se sono a meno di 5 pixel dall'eventuale cursore dati, cambio poi il cursore mouse per indicare la trascinabilità del cursore dati:
    ...
    if(dataCurs2Visible){
      if(dataCurs2Dragging){
        setCursor(Qt::SizeHorCursor);
        values=giveValues(x-1, linearInterpolate, nearX, true, true);
...
//        if(lastAutoScale==false){
//           exactMatch=true;
//           scaleXY(dispRect,true);
//        }else{  //last autoScale=true
//           exactMatch=false;
//            scaleXY(dispRect,false);
//        }
  ...
  if(axis.type==atX){
    ...
    for(x=X0+margin, auxd=X0+margin, xf=xAxis.scaleMin; x<=X1-margin;
      ...
      if(ticCount/2*2==ticCount){
        //Scrivo la tacca numerica, omettendola se c'è poco spazio sper la label di asse e sto scrivendo la penultima tacca.  Individuo che si tratta della penultima tacca con la condizione che la x è compresa fra X1-margin-1.5*ticinterv.y e X1-Margin-0.5*ticinterv.x
        if (!removeOneNumLbl)
 ...
 * Naturalmente esse potrenno essere usate solo se le unità relative ai vari plot del
 ...
 *
 * In tutti i casi in cui ci sono label numeriche se i numeri sono molto grandi o molto
 * piccoli invece delle potenze di 10 vengono usati i prefissi (elencati qui sotto
 * sotto "prefix").
 *
...
//     hAdjust=atLeft;
 ...
 /***  Fase 2: Determino sulla base di useExternalUnits e useUserUnits  ***/
 if(useUserUnits){
   switch(axis.type){
 ...
 if(!autoLabelXY || abs(axis.scaleExponent)>12 ){
   unitS="";
   goto Escape;
 }
...
if(axis.type==atX)
  ...
  /***  Fase 3: Determino nei vari casi base ed esponente ***/
Escape:
  if(axis.scaleType==stDB)
    unitS="dB";
  ...
  // A questo punto è determinato se unit è "" (label in potenza di 10 e solo quando necessario) ovvero !"" (label testuale).  Posso fissare base ed esponente della label nei due casi.
  if(unitS==""){
    if(axis.scaleFactor==1){
      ;
    } else {
      unitS="*10"+QString::number(axis.scaleExponent);
    }
  }else{ //qui ho un'unità di misura e premetto il prefisso
...
//Write:
  ...
  if (useSmartUnits)
    ...
    return drawText2(X,Y,hAdjust,vAdjust,unitS,"",useBrackets, _virtual)+1;
  }
}
